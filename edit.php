...
require_once($CFG->dirroot . '/mod/extendedquiz/editlib.php');
require_once($CFG->dirroot . '/mod/extendedquiz/addrandomform.php');
...
$quizhasattempts = extendedquiz_has_attempts($quiz->id);
...
$questionbank = new extendedquiz_question_bank_view($contexts, $thispageurl, $course, $cm, $quiz);
$questionbank->set_quiz_has_attempts($quizhasattempts);

// Log this visit.
add_to_log($cm->course, 'extendedquiz', 'editquestions',
...
if ($quiz->shufflequestions) {
    ...
    $quiz->questions = extendedquiz_repaginate($quiz->questions, 0);
}
...
if (($up = optional_param('up', false, PARAM_INT)) && confirm_sesskey()) {
    $quiz->questions = extendedquiz_move_question_up($quiz->questions, $up);
...
if (($down = optional_param('down', false, PARAM_INT)) && confirm_sesskey()) {
    $quiz->questions = extendedquiz_move_question_down($quiz->questions, $down);
    $DB->set_field('extendedquiz', 'questions', $quiz->questions, array('id' => $quiz->id));
    extendedquiz_delete_previews($quiz);
    redirect($afteractionurl);
}
...
if (optional_param('repaginate', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    $quiz->questions = extendedquiz_repaginate($quiz->questions, $questionsperpage );
    $DB->set_field('extendedquiz', 'questions', $quiz->questions, array('id' => $quiz->id));
    extendedquiz_delete_previews($quiz);
    redirect($afteractionurl);
}
...
if (($addquestion = optional_param('addquestion', 0, PARAM_INT)) && confirm_sesskey()) {
    ...
    extendedquiz_require_question_use($addquestion);
    $addonpage = optional_param('addonpage', 0, PARAM_INT);
    extendedquiz_add_quiz_question($addquestion, $quiz, $addonpage);
    extendedquiz_delete_previews($quiz);
...
if (optional_param('add', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    foreach ($rawdata as $key => $value) { // Parse input for question ids.
        if (preg_match('!^q([0-9]+)$!', $key, $matches)) {
            $key = $matches[1];
            extendedquiz_require_question_use($key);
            extendedquiz_add_quiz_question($key, $quiz);
        }
    }
    extendedquiz_delete_previews($quiz);
    extendedquiz_update_sumgrades($quiz);
...
if ((optional_param('addrandom', false, PARAM_BOOL)) && confirm_sesskey()) {
    ...
    extendedquiz_add_random_questions($quiz, $addonpage, $categoryid, $randomcount, $recurse);
...
if (optional_param('addnewpagesafterselected', null, PARAM_CLEAN) &&
    ...
    foreach ($selectedquestionids as $questionid) {
        $quiz->questions = extendedquiz_add_page_break_after($quiz->questions, $questionid);
    }
    $DB->set_field('extendedquiz', 'questions', $quiz->questions, array('id' => $quiz->id));
    extendedquiz_delete_previews($quiz);
    redirect($afteractionurl);
...
if ($addpage !== false && confirm_sesskey()) {
    $quiz->questions = extendedquiz_add_page_break_at($quiz->questions, $addpage);
    $DB->set_field('extendedquiz', 'questions', $quiz->questions, array('id' => $quiz->id));
    extendedquiz_delete_previews($quiz);
    redirect($afteractionurl);
}
...
if (($deleteemptypage !== false) && confirm_sesskey()) {
    $quiz->questions = extendedquiz_delete_empty_page($quiz->questions, $deleteemptypage);
    $DB->set_field('extendedquiz', 'questions', $quiz->questions, array('id' => $quiz->id));
    extendedquiz_delete_previews($quiz);
    redirect($afteractionurl);
}
...
if ($remove && confirm_sesskey()) {
    ...
    if ($DB->record_exists('question', array('id' => $remove))) {
        extendedquiz_require_question_use($remove);
    }
    extendedquiz_remove_question($quiz, $remove);
    extendedquiz_delete_previews($quiz);
    extendedquiz_update_sumgrades($quiz);
    redirect($afteractionurl);
}
...
if (optional_param('quizdeleteselected', false, PARAM_BOOL) &&
        !empty($selectedquestionids) && confirm_sesskey()) {
    foreach ($selectedquestionids as $questionid) {
        if (extendedquiz_has_question_use($questionid)) {
            extendedquiz_remove_question($quiz, $questionid);
        }
    }
    extendedquiz_delete_previews($quiz);
    extendedquiz_update_sumgrades($quiz);
...
if (optional_param('savechanges', false, PARAM_BOOL) && confirm_sesskey()) {
    ...
    if ($maxgrade >= 0) {
        extendedquiz_set_grade($maxgrade, $quiz);
    }
    ...
    if ($recomputesummarks) {
        ...
        extendedquiz_update_grades($quiz, 0, true);
    }
    redirect($afteractionurl);
}
...
$numberoflisteners = max(extendedquiz_number_of_pages($quiz->questions), 1);
...
if ($quiz->shufflequestions) {
    ...
    $quiz->questions = extendedquiz_repaginate($quiz->questions, $quiz->questionsperpage);
} else {
...
$notifystrings = array();
if ($quizhasattempts) {
    $reviewlink = extendedquiz_attempt_summary_link_to_reports($quiz, $cm, $contexts->lowest());
...
if ($quiz->shufflequestions) {
    ...
    $updatelink = '<a href="'.$updateurl->out().'">' . get_string('updatethis', '',
            get_string('modulename', 'extendedquiz')) . '</a>';
...
if (!$quiz_reordertool && $canaddrandom) {
    $randomform = new extendedquiz_add_random_form(new moodle_url('/mod/extendedquiz/addrandom.php'), $contexts);
