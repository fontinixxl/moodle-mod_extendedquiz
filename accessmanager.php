 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class extendedquiz_access_manager {
    /** @var extendedquiz the quiz settings object. */
    protected $quizobj;
     ...
     * @param extendedquiz $quizobj information about the quiz in question.
    ...
    protected function make_rules($quizobj, $timenow, $canignoretimelimits) {
        ...
        foreach ($superceededrules as $superceededrule) {
            unset($rules['extendedquizaccess_' . $superceededrule]);
        }
    ...
    protected static function get_rule_classes() {
        return get_plugin_list_with_class('extendedquizaccess', '', 'rule.php');
    }
     ...
     *
     * @param mod_extendedquiz_mod_form $quizform the quiz settings form that is being built.
     * @param MoodleQuickForm $mform the wrapped MoodleQuickForm.
     */
    public static function add_settings_form_fields(
            mod_extendedquiz_mod_form $quizform, MoodleQuickForm $mform) {
     ...
     * @param mod_extendedquiz_mod_form $quizform the quiz form object.
     * @return array $errors the updated $errors array.
     */
    public static function validate_settings_form_fields(array $errors,
            array $data, $files, mod_extendedquiz_mod_form $this) {
     ...
     *
     * @param int $quizid the quiz id.
     * @return object mdl_extendedquiz row with extra fields.
     */
     ...
     * @param int|null $attemptid the id of the current attempt, if there is one,
     *      otherwise null.
     * @return mod_extendedquiz_preflight_check_form the form.
     */
    public function get_preflight_check_form(moodle_url $url, $attemptid) {
        return new mod_extendedquiz_preflight_check_form($url->out_omit_querystring(),
     ...
     *
     * @param mod_extendedquiz_renderer $output the quiz renderer.
    ...
    public function make_review_link($attempt, $reviewoptions, $output) {
        ...
        if (in_array($attempt->state, array(extendedquiz_attempt::IN_PROGRESS, extendedquiz_attempt::OVERDUE))) {
            return $output->no_review_message('');
        }

        $when = extendedquiz_attempt_state($this->quizobj->get_quiz(), $attempt);
        $reviewoptions = mod_extendedquiz_display_options::make_from_quiz(
