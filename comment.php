...
$PAGE->set_url('/mod/extendedquiz/comment.php', array('attempt' => $attemptid, 'slot' => $slot));

$attemptobj = extendedquiz_attempt::create($attemptid);

// Can only grade finished attempts.
if (!$attemptobj->is_finished()) {
    print_error('attemptclosed', 'extendedquiz');
}

// Check login and permissions.
require_login($attemptobj->get_course(), false, $attemptobj->get_cm());
$attemptobj->require_capability('mod/extendedquiz:grade');

// Log this action.
add_to_log($attemptobj->get_courseid(), 'extendedquiz', 'manualgrade', 'comment.php?attempt=' .
...
echo '<form method="post" class="mform" id="manualgradingform" action="' .
        $CFG->wwwroot . '/mod/extendedquiz/comment.php">';
        ...
        <div class="fitem fitem_actionbuttons fitem_fsubmit">
            <fieldset class="felement fsubmit">
                <input id="id_submitbutton" type="submit" name="submit" value="<?php
                        print_string('save', 'extendedquiz'); ?>"/>
            </fieldset>
        </div>
    </div>
</fieldset>
<?php
echo '</form>';
$PAGE->requires->js_init_call('M.mod_extendedquiz.init_comment_popup', null, false, extendedquiz_get_js_module());
