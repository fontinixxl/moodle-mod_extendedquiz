...
$PAGE->set_url('/mod/extendedquiz/index.php', array('id'=>$id));
...
add_to_log($course->id, "extendedquiz", "view all", "index.php?id=$course->id", "");

// Print the header.
$strquizzes = get_string("modulenameplural", "extendedquiz");
$streditquestions = '';
...
if (!$quizzes = get_all_instances_in_course("extendedquiz", $course)) {
...
foreach ($quizzes as $quiz) {
    ...
    if (extendedquiz_has_feedback($quiz)) {
        $showfeedback=true;
    }
...
if (has_capability('mod/extendedquiz:viewreports', $coursecontext)) {
    ...
    $grades = $DB->get_records_sql_menu('
            ...
            JOIN {extendedquiz} q ON q.id = qg.quiz
...
foreach ($quizzes as $quiz) {
    ...
    if ($showing == 'stats') {
        ...
        $data[] = extendedquiz_attempt_summary_link_to_reports($quiz, $cm, $context);

    } else if ($showing == 'grades') {
        // Grade and feedback.
        $attempts = extendedquiz_get_user_attempts($quiz->id, $USER->id, 'all');
        list($someoptions, $alloptions) = extendedquiz_get_combined_reviewoptions(
        ...
        if ($quiz->grade && array_key_exists($quiz->id, $grades)) {
            if ($alloptions->marks >= question_display_options::MARK_AND_MAX) {
                $a = new stdClass();
                $a->grade = extendedquiz_format_grade($quiz, $grades[$quiz->id]);
                $a->maxgrade = extendedquiz_format_grade($quiz, $quiz->grade);
                $grade = get_string('outofshort', 'quiz', $a);
            }
            if ($alloptions->overallfeedback) {
                $feedback = extendedquiz_feedback_for_grade($grades[$quiz->id], $quiz, $context);
            }
        }
        $data[] = $grade;
        if ($showfeedback) {
