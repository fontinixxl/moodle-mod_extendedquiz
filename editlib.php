 ...
 */
function extendedquiz_require_question_use($questionid) {
    global $DB;
 ...
 */
function extendedquiz_has_question_use($questionid) {
    global $DB;
    $question = $DB->get_record('question', array('id' => $questionid), '*', MUST_EXIST);
 ...
 */
function extendedquiz_remove_question($quiz, $questionid) {
    global $DB;
    ...
    $DB->set_field('extendedquiz', 'questions', $quiz->questions, array('id' => $quiz->id));
    $DB->delete_records('extendedquiz_q_instances',
 ...
 */
function extendedquiz_delete_empty_page($layout, $index) {
 ...
 */
function extendedquiz_add_quiz_question($id, $quiz, $page = 0) {
    global $DB;
    $questions = explode(',', extendedquiz_clean_layout($quiz->questions));
    ...
    if (is_int($page) && $page >= 1) {
        $numofpages = extendedquiz_number_of_pages($quiz->questions);
        if ($numofpages<$page) {
    ...
    $DB->insert_record('extendedquiz_q_instances', $instance);
}

function extendedquiz_add_random_questions($quiz, $addonpage, $categoryid, $number,
    ...
    if ($existingquestions = $DB->get_records_sql(
        ...
        while (($existingquestion = array_shift($existingquestions)) && $number > 0) {
            extendedquiz_add_quiz_question($existingquestion->id, $quiz, $addonpage);
            $number -= 1;
        }
    }
    ...
    for ($i = 0; $i < $number; $i += 1) {
        ...
        extendedquiz_add_quiz_question($question->id, $quiz, $addonpage);
    }
}
 ...
 * @return the updated layout
 */
function extendedquiz_add_page_break_at($layout, $index) {
    $questionids = explode(',', $layout);
 ...
 * @return the updated layout
 */
function extendedquiz_add_page_break_after($layout, $questionid) {
    $questionids = explode(',', $layout);
    $key = array_search($questionid, $questionids);
 ...
 */
function extendedquiz_save_new_layout($quiz) {
    global $DB;
    $DB->set_field('extendedquiz', 'questions', $quiz->questions, array('id' => $quiz->id));
    extendedquiz_delete_previews($quiz);
    extendedquiz_update_sumgrades($quiz);
}
...
function extendedquiz_update_question_instance($grade, $questionid, $quiz, $penalty = false, $nattempts = false ) {
    ...
    $instance = $DB->get_record('extendedquiz_q_instances', array('quiz' => $quiz->id,
            'question' => $questionid));
    $slot = extendedquiz_get_slot_for_question($quiz, $questionid);
...
function _extendedquiz_move_question($layout, $questionid, $shift) {
 ...
 * @return the updated layout
 */
function extendedquiz_move_question_up($layout, $questionid) {
    return _extendedquiz_move_question($layout, $questionid, -1);
}
 ...
 * @return the updated layout
 */
function extendedquiz_move_question_down($layout, $questionid) {
    return _extendedquiz_move_question($layout, $questionid, +1);
}
 ...
 */
function extendedquiz_print_question_list($quiz, $pageurl, $allowdelete, $reordertool,
    ...
    if ($quiz->questions) {
        ...
        $questions = $DB->get_records_sql("SELECT q.*, qc.contextid, qqi.grade as maxmark
                              ...
                              JOIN {extendedquiz_q_instances} qqi ON qqi.question = q.id
                             WHERE q.id $usql AND qqi.quiz = ?", $params);
    } else {
        $questions = array();
    }

    $layout = extendedquiz_clean_layout($quiz->questions);
...
<div class="qorder">
            ...
            <div class="questioncontentcontainer">
                ...
                if ($question->qtype == 'random') { // It is a random question.
                    if (!$reordertool) {
                        extendedquiz_print_randomquestion($question, $pageurl, $quiz, $quiz_qbanktool);
                    } else {
                        extendedquiz_print_randomquestion_reordertool($question, $pageurl, $quiz);
                    }
                } else { // It is a single question.
                    if (!$reordertool) {
                        extendedquiz_print_singlequestion($question, $returnurl, $quiz);
                    } else {
                        extendedquiz_print_singlequestion_reordertool($question, $returnurl, $quiz);
                    }
                }
                ?>
            </div>
        ...
        if ($qnum == 0) {
            if ($pageopen) {
                if (!$reordertool && !($quiz->shufflequestions &&
                    ...
                    extendedquiz_print_pagecontrols($quiz, $pageurl, $pagecount,
 ...
 * @param bool $canaddrandom is the user able to add random questions anywere?
 */
function extendedquiz_print_pagecontrols($quiz, $pageurl, $page, $hasattempts,
    ...
    <div class="singlebutton">
        <form class="randomquestionform" action="<?php echo $CFG->wwwroot;
                ?>/mod/extendedquiz/addrandom.php" method="get">
            <div>
 ...
 */
function extendedquiz_print_singlequestion($question, $returnurl, $quiz) {
    
    global $OUTPUT;
    
    echo '<div class="singlequestion ' . $question->qtype . '">';
    echo extendedquiz_question_edit_button($quiz->cmid, $question, $returnurl,
            extendedquiz_question_tostring($question) . ' ');
    ...
    if($question->qtype == 'programmedresp'){
        ...
        if(extendedquiz_get_question_extendedquiz_args($question->id)){
            
            
            $url = new moodle_url('/mod/extendedquiz/assignvars.php',
                    array('quizid' => $quiz->id, 'questionid' => $question->id));
            
            $strassignvars = get_string('assignvars', 'extendedquiz');
            $image = $OUTPUT->pix_icon('i/payment', $strassignvars);
            
            $action = new popup_action('click', $url,
            question_preview_popup_params());
            
            echo '<span class="questionpreview">' .
                    $OUTPUT->action_link($url, $image, $action, array('title' => $strassignvars)).
                 '</span>';    
 ...
 */
function extendedquiz_print_randomquestion(&$question, &$pageurl, &$quiz, $quiz_qbanktool) {
    global $DB, $OUTPUT;
    ...
    echo print_question_icon($question);
    guidedprint_random_option_icon($question);
    ...
    if ($questioncount == 0) {
        ...
        foreach ($questionstoshow as $question) {
            echo '<li>' . extendedquiz_question_tostring($question, true) . '</li>';
        }
 ...
 * @param object $quiz The quiz in the context of which the question is being displayed
 */
function extendedquiz_print_singlequestion_reordertool($question, $returnurl, $quiz) {
    echo '<div class="singlequestion ' . $question->qtype . '">';
    echo '<label for="s' . $question->id . '">';
    echo print_question_icon($question);
    echo ' ' . extendedquiz_question_tostring($question);
    echo '</label>';
    echo '<span class="questionpreview">' .
            extendedquiz_question_action_icons($quiz, $quiz->cmid, $question, $returnurl) . '</span>';
    echo "</div>\n";
}
 ...
 * @param object $quiz The quiz in the context of which the question is being displayed
 */
function extendedquiz_print_randomquestion_reordertool(&$question, &$pageurl, &$quiz) {
    global $DB, $OUTPUT;
    ...
    echo '<span class="questionpreview">';
    echo extendedquiz_question_preview_button($quiz, $question, false);
    echo '</span>';
    echo "</div>";
 ...
 */
function guidedprint_random_option_icon($question) {
    global $OUTPUT;
 ...
 */
function extendedquiz_question_tostring($question, $showicon = false,
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class question_bank_add_to_extendedquiz_action_column extends question_bank_action_column_base {
    protected $stradd;
...
class question_bank_question_name_text_column extends question_bank_question_name_column {
    ...
    protected function display_content($question, $rowclasses) {
        ...
        echo extendedquiz_question_tostring($question, false, true, true);
        if ($labelfor) {
            echo '</label>';
        }
        echo '</div>';
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class extendedquiz_question_bank_view extends question_bank_view {
    ...
    protected function known_field_types() {
        ...
        $types[] = new question_bank_add_to_extendedquiz_action_column($this);
    ...
    public function preview_question_url($question) {
        return extendedquiz_question_preview_url($this->quiz, $question);
    }
    ...
    public function add_to_quiz_url($questionid) {
        ...
        return new moodle_url('/mod/extendedquiz/edit.php', $params);
    }
 ...
 */
function extendedquiz_print_grading_form($quiz, $pageurl, $tabindex) {
    global $OUTPUT;
    $strsave = get_string('save', 'quiz');
 ...
 * @param object $quiz The quiz object of the quiz in question
 */
function extendedquiz_print_status_bar($quiz) {
    global $CFG;

    $bits = array();

    $bits[] = html_writer::tag('span',
            get_string('totalmarksx', 'quiz', extendedquiz_format_grade($quiz, $quiz->sumgrades)),
            array('class' => 'totalpoints'));

    $bits[] = html_writer::tag('span',
            get_string('numquestionsx', 'quiz', extendedquiz_number_of_questions_in_quiz($quiz->questions)),
