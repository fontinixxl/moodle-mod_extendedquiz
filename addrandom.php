...
list($thispageurl, $contexts, $cmid, $cm, $quiz, $pagevars) =
        question_edit_setup('editq', '/mod/extendedquiz/addrandom.php', true);
...
if ($returnurl) {
    ...
    $returnurl = new moodle_url('/mod/extendedquiz/edit.php', array('cmid' => $cmid));
}
if ($scrollpos) {
...
$mform = new extendedquiz_add_random_form(new moodle_url('/mod/extendedquiz/addrandom.php'), $contexts);
...
if ($data = $mform->get_data()) {
    if (!empty($data->existingcategory)) {
        ...
        add_to_log($quiz->course, 'extendedquiz', 'addcategory',
    ...
    extendedquiz_add_random_questions($quiz, $addonpage, $categoryid, 1, $includesubcategories);
...
// Setup $PAGE.
$streditingquiz = get_string('editinga', 'moodle', get_string('modulename', 'extendedquiz'));
...
echo $OUTPUT->heading(get_string('addrandomquestiontoquiz', 'extendedquiz', $quizname), 2, 'mdl-left');
$mform->display();
echo $OUTPUT->footer();
