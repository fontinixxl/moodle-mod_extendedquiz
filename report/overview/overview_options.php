...
/**
 * Class to store the options for a {@link extendedquiz_overview_report}.
 *
 * @copyright 2012 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class extendedquiz_overview_options extends mod_extendedquiz_attempts_report_options {
    ...
    public function setup_from_user_preferences() {
        ...
        $this->slotmarks = get_user_preferences('extendedquiz_report_overview_detailedmarks', $this->slotmarks);        //?¿?¿
    }

    public function update_user_preferences() {
        parent::update_user_preferences();

        if (extendedquiz_has_grades($this->quiz)) {
            set_user_preference('extendedquiz_overview_slotmarks', $this->slotmarks);
        }
    }
    ...
    public function resolve_dependencies() {
        ...
        $this->checkboxcolumn = has_any_capability(
                array('mod/extendedquiz:regrade', 'mod/extendedquiz:deleteattempts'), context_module::instance($this->cm->id))
