 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class mod_extendedquiz_attempts_report_form extends moodleform {
    ...
    protected function standard_attempt_fields(MoodleQuickForm $mform) {
        ...
        $mform->addElement('select', 'attempts', get_string('reportattemptsfrom', 'quiz'), array(
                    extendedquiz_attempts_report::ENROLLED_WITH    => get_string('reportuserswith', 'quiz'),
                    extendedquiz_attempts_report::ENROLLED_WITHOUT => get_string('reportuserswithout', 'quiz'),
                    extendedquiz_attempts_report::ENROLLED_ALL     => get_string('reportuserswithorwithout', 'quiz'),
                    extendedquiz_attempts_report::ALL_WITH        => get_string('reportusersall', 'quiz'),
                 ));
        ...
        $mform->disabledIf('stateinprogress', 'attempts', 'eq', extendedquiz_attempts_report::ENROLLED_WITHOUT);
        $mform->disabledIf('stateoverdue',    'attempts', 'eq', extendedquiz_attempts_report::ENROLLED_WITHOUT);
        $mform->disabledIf('statefinished',   'attempts', 'eq', extendedquiz_attempts_report::ENROLLED_WITHOUT);
        $mform->disabledIf('stateabandoned',  'attempts', 'eq', extendedquiz_attempts_report::ENROLLED_WITHOUT);

        if (extendedquiz_report_can_filter_only_graded($this->_customdata['quiz'])) {
            $gm = html_writer::tag('span',
                    extendedquiz_get_grading_option_name($this->_customdata['quiz']->grademethod),
            ...
            $mform->disabledIf('onlygraded', 'attempts', 'eq', extendedquiz_attempts_report::ENROLLED_WITHOUT);
