...
/**
 * Base class for extendedquiz reports that are basically a table with one row for each attempt.
 *
 * @copyright 2010 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class extendedquiz_attempts_report extends extendedquiz_default_report {
    ...
    /** @var mod_extendedquiz_attempts_report_form The settings form to use. */
    protected $form;
    ...
    protected function init($mode, $formclass, $quiz, $cm, $course) {
        ...
        $this->qmsubselect = extendedquiz_report_qm_filter_select($quiz);
    ...
    protected function add_grade_columns($quiz, $usercanseegrades, &$columns, &$headers, $includefeedback = true) {
        ...
        if ($includefeedback && extendedquiz_has_feedback($quiz)) {
     ...
     * @param mod_extendedquiz_attempts_report_options $options the display options.
    ...
    protected function set_up_table_columns($table, $columns, $headers, $reporturl,
            mod_extendedquiz_attempts_report_options $options, $collapsible) {
        ...
        if (empty($currentgroup) || $groupstudents) {
            if (optional_param('delete', 0, PARAM_BOOL) && confirm_sesskey()) {
                if ($attemptids = optional_param_array('attemptid', array(), PARAM_INT)) {
                    require_capability('mod/extendedquiz:deleteattempts', $this->context);
    ...
    protected function delete_selected_attempts($quiz, $cm, $attemptids, $allowed) {
        ...
        foreach ($attemptids as $attemptid) {
            $attempt = $DB->get_record('extendedquiz_attempts', array('id' => $attemptid));
            ...
            add_to_log($quiz->course, 'extendedquiz', 'delete attempt', 'report.php?id=' . $cm->id,
                    $attemptid, $cm->id);
