 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class extendedquiz_statistics_table extends flexible_table {
    ...
    public function __construct() {
        parent::__construct('mod-extendedquiz-report-statistics-report');
    }
    ...
    public function statistics_setup($quiz, $cmid, $reporturl, $s) {
        ...
        $columns[] = 'number';
        $headers[] = get_string('questionnumber', 'extendedquiz_statistics');
        ...
        if (!$this->is_downloading()) {
            ...
            $columns[] = 'qtype';
            $headers[] = get_string('questiontype', 'extendedquiz_statistics');
        }
        ...
        $columns[] = 's';
        $headers[] = get_string('attempts', 'extendedquiz_statistics');

        if ($s > 1) {
            $columns[] = 'facility';
            $headers[] = get_string('facility', 'extendedquiz_statistics');

            $columns[] = 'sd';
            $headers[] = get_string('standarddeviationq', 'extendedquiz_statistics');
        }

        $columns[] = 'random_guess_score';
        $headers[] = get_string('random_guess_score', 'extendedquiz_statistics');

        $columns[] = 'intended_weight';
        $headers[] = get_string('intended_weight', 'extendedquiz_statistics');

        $columns[] = 'effective_weight';
        $headers[] = get_string('effective_weight', 'extendedquiz_statistics');

        $columns[] = 'discrimination_index';
        $headers[] = get_string('discrimination_index', 'extendedquiz_statistics');

        $columns[] = 'discriminative_efficiency';
        $headers[] = get_string('discriminative_efficiency', 'extendedquiz_statistics');
    ...
    protected function col_actions($question) {
        return extendedquiz_question_action_icons($this->quiz, $this->cmid, $question, $this->baseurl);
    }
    ...
    protected function col_name($question) {
        ...
        if ($url) {
            $name = html_writer::link($url, $name,
                    array('title' => get_string('detailedanalysis', 'extendedquiz_statistics')));
        }
    ...
    protected function col_intended_weight($question) {
        return extendedquiz_report_scale_summarks_as_percentage(
    ...
    protected function col_effective_weight($question) {
        ...
        if ($question->_stats->negcovar) {
            $negcovar = get_string('negcovar', 'extendedquiz_statistics');

            if (!$this->is_downloading()) {
                $negcovar = html_writer::tag('div',
                        $negcovar . $OUTPUT->help_icon('negcovar', 'extendedquiz_statistics'),
