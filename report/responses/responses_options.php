...
require_once($CFG->dirroot . '/mod/extendedquiz/report/attemptsreport_options.php');


/**
 * Class to store the options for a {@link extendedquiz_responses_report}.
 *
 * @copyright 2012 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class extendedquiz_responses_options extends mod_extendedquiz_attempts_report_options {
    ...
    public function setup_from_user_preferences() {
        ...
        $this->showqtext     = get_user_preferences('extendedquiz_report_responses_qtext', $this->showqtext);
        $this->showresponses = get_user_preferences('extendedquiz_report_responses_resp',  $this->showresponses);
        $this->showright     = get_user_preferences('extendedquiz_report_responses_right', $this->showright);
    }

    public function update_user_preferences() {
        parent::update_user_preferences();

        set_user_preference('extendedquiz_report_responses_qtext', $this->showqtext);
        set_user_preference('extendedquiz_report_responses_resp',  $this->showresponses);
        set_user_preference('extendedquiz_report_responses_right', $this->showright);
    }
    ...
    public function resolve_dependencies() {
        ...
        $this->checkboxcolumn = has_capability('mod/extendedquiz:deleteattempts', context_module::instance($this->cm->id))
                && ($this->attempts != extendedquiz_attempts_report::ENROLLED_WITHOUT);
    }
}
