...
require_once($CFG->dirroot . '/mod/extendedquiz/report/grading/gradingsettings_form.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class extendedquiz_grading_report extends extendedquiz_default_report {
    ...
    public function display($quiz, $cm, $course) {
        ...
        require_capability('mod/extendedquiz:grade', $this->context);
        ...
        $this->questions = extendedquiz_report_get_significant_questions($quiz);
        if ($slot && !array_key_exists($slot, $this->questions)) {
            throw new moodle_exception('unknownquestion', 'extendedquiz_grading');
        }
        ...
        $questionsinquiz = extendedquiz_questions_in_quiz($quiz->questions);
        $counts = null;
        if ($slot && $questionsinquiz) {
            ...
            if (!$counts || $counts->$grade == 0) {
                redirect($this->list_questions_url(), get_string('alldoneredirecting', 'extendedquiz_grading'));
            }
        }
    ...
    protected function get_qubaids_condition() {
        ...
        $params = array('mangrquizid' => $this->cm->instance, 'statefinished' => extendedquiz_attempt::FINISHED);

        $currentgroup = groups_get_activity_group($this->cm, true);
        if ($currentgroup) {
            $users = get_users_by_capability($this->context,
                    array('mod/extendedquiz:reviewmyattempts', 'mod/extendedquiz:attempt'), 'u.id, u.id', '', '', '',
                    $currentgroup, '', false);
            if (empty($users)) {
        ...
        return new qubaid_join('{extendedquiz_attempts} quiza', 'quiza.uniqueid', $where, $params);
    }
    ...
    protected function load_attempts_by_usage_ids($qubaids) {
        ...
        $params[] = extendedquiz_attempt::FINISHED;
        $params[] = $this->quiz->id;
    ...
    protected function base_url() {
        return new moodle_url('/mod/extendedquiz/report.php',
    ...
    protected function format_count_for_table($counts, $type, $gradestring) {
        ...
        if ($counts->$type > 0) {
            $result .= ' ' . html_writer::link($this->grade_question_url(
                    ...
                    get_string($gradestring, 'extendedquiz_grading'),
    ...
    protected function display_index($includeauto) {
        ...
        echo $OUTPUT->heading(get_string('questionsthatneedgrading', 'extendedquiz_grading'));
        if ($includeauto) {
            $linktext = get_string('hideautomaticallygraded', 'extendedquiz_grading');
        } else {
            $linktext = get_string('alsoshowautomaticallygraded', 'extendedquiz_grading');
        }
        ...
        if (empty($data)) {
            echo $OUTPUT->heading(get_string('nothingfound', 'extendedquiz_grading'));
            return;
        }
        ...
        $table->head[] = get_string('qno', 'extendedquiz_grading');
        $table->head[] = get_string('questionname', 'extendedquiz_grading');
        $table->head[] = get_string('tograde', 'extendedquiz_grading');
        $table->head[] = get_string('alreadygraded', 'extendedquiz_grading');
        if ($includeauto) {
            $table->head[] = get_string('automaticallygraded', 'extendedquiz_grading');
        }
        $table->head[] = get_string('total', 'extendedquiz_grading');
    ...
    protected function display_grading_interface($slot, $questionid, $grade,
        ...
        $mform = new extendedquiz_grading_settings_form($hidden, $counts, $shownames, $showidnumbers);
        ...
        echo $OUTPUT->heading(get_string('gradingquestionx', 'extendedquiz_grading', $a));
        echo html_writer::tag('p', html_writer::link($this->list_questions_url(),
                get_string('backtothelistofquestions', 'extendedquiz_grading')),
        ...
        $a->of = $count;
        echo $OUTPUT->heading(get_string('gradingattemptsxtoyofz', 'extendedquiz_grading', $a), 3);
        ...
        foreach ($qubaids as $qubaid) {
            ...
            $displayoptions = extendedquiz_get_review_options($this->quiz, $attempt, $this->context);
        ...
        echo html_writer::tag('div', html_writer::empty_tag('input', array(
                'type' => 'submit', 'value' => get_string('saveandnext', 'extendedquiz_grading'))),
                array('class' => 'mdl-align')) .
    ...
    protected function get_question_heading($attempt, $shownames, $showidnumbers) {
        ...
        if ($shownames && $showidnumbers) {
            return get_string('gradingattemptwithidnumber', 'extendedquiz_grading', $a);
        } else if ($shownames) {
            return get_string('gradingattempt', 'extendedquiz_grading', $a);
    ...
    protected function process_submitted_data() {
        ...
        foreach ($qubaids as $qubaid) {
            $attempt = $attempts[$qubaid];
            $attemptobj = new extendedquiz_attempt($attempt, $this->quiz, $this->cm, $this->course);
