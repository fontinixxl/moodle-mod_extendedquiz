...
/**
 * Upgrade script for the extendedquiz module.
 *
 * @package    mod
 ...
 */
function xmldb_extendedquiz_upgrade($oldversion) {
    global $CFG, $DB;
    ...
    if ($oldversion < 2011120700) {
        ...
        $table = new xmldb_table('extendedquiz_reports');
        ...
        upgrade_mod_savepoint(true, 2011120700, 'extendedquiz');
    }
    ...
    if ($oldversion < 2011120701) {
        ...
        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2011120701, 'extendedquiz');
    }

    if ($oldversion < 2011120703) {
        // Track page of quiz attempts.
        $table = new xmldb_table('extendedquiz_attempts');
        ...
        upgrade_mod_savepoint(true, 2011120703, 'extendedquiz');
    }

    if ($oldversion < 2012030901) {
        // Configuration option for navigation method.
        $table = new xmldb_table('extendedquiz');
        ...
        upgrade_mod_savepoint(true, 2012030901, 'extendedquiz');
    }
    ...
    if ($oldversion < 2012040198) {
        ...
        $contextids = $DB->get_records_sql_menu("
                SELECT DISTINCT qu.contextid, 1
                  ...
                  JOIN {extendedquiz_attempts} quiza ON quiza.uniqueid = qu.id
            ...
            foreach ($fileareas as $filearea) {
                ...
                $fs->delete_area_files_select($contextid, 'question', $filearea,
                        "IN (SELECT qas.id
                               ...
                               JOIN {extendedquiz_attempts} quiza ON quiza.uniqueid = qa.questionusageid
                              WHERE quiza.preview = 1)");
            }
        }
        ...
        $DB->delete_records_select('question_attempts', "{question_attempts}.questionusageid IN (
                SELECT uniqueid FROM {extendedquiz_attempts} WHERE preview = 1)");

        $DB->delete_records_select('question_usages', "{question_usages}.id IN (
                SELECT uniqueid FROM {extendedquiz_attempts} WHERE preview = 1)");

        // Finally delete the previews.
        $DB->delete_records('extendedquiz_attempts', array('preview' => 1));

        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2012040198, 'extendedquiz');
    }
    ...
    if ($oldversion < 2012040199) {
        ...
        foreach ($problems as $problem) {
            ...
            if ($attempt != $problem->attempt) {
                $DB->set_field('extendedquiz_attempts', 'attempt', $attempt, array('id' => $problem->id));
            }
            $attempt += 1;
        }
        ...
        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2012040199, 'extendedquiz');
    }
    ...
    if ($oldversion < 2012040200) {
        ...
        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2012040200, 'extendedquiz');
    }
    ...
    if ($oldversion < 2012040201) {
        ...
        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2012040201, 'extendedquiz');
    }
    ...
    if ($oldversion < 2012040202) {
        ...
        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2012040202, 'extendedquiz');
    }
    ...
    if ($oldversion < 2012040203) {
        ...
        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2012040203, 'extendedquiz');
    }

    if ($oldversion < 2012040204) {

        // Update quiz_attempts.state for finished attempts.
        $DB->set_field_select('extendedquiz_attempts', 'state', 'finished', 'timefinish > 0');

        // Other, more complex transitions (basically abandoned attempts), will
        // be handled by cron later.

        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2012040204, 'extendedquiz');
    }
    ...
    if ($oldversion < 2012040205) {
        ...
        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2012040205, 'extendedquiz');
    }
    ...
    if ($oldversion < 2012040206) {
        ...
        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2012040206, 'extendedquiz');
    }
    ...
    if ($oldversion < 2012061702) {
        ...
        $DB->delete_records('extendedquiz_q_instances', array('question' => 0));

        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2012061702, 'extendedquiz');
    }
    ...
    if ($oldversion < 2012061703) {
        ...
        $DB->set_field('extendedquiz', 'questiondecimalpoints', -1, array('questiondecimalpoints' => -2));

        // Quiz savepoint reached.
        upgrade_mod_savepoint(true, 2012061703, 'extendedquiz');
    }
    ...
    if ($oldversion < 2012100801) {
        ...
        unset_config('overduelastrun', 'extendedquiz');
        unset_config('overduedoneto', 'extendedquiz');

        // Update timecheckstate on all open attempts
        require_once($CFG->dirroot . '/mod/extendedquiz/locallib.php');
        extendedquiz_update_open_attempts(array());

        // quiz savepoint reached
        upgrade_mod_savepoint(true, 2012100801, 'extendedquiz');
    }
    ...
    if ($oldversion < 2012112901) {
        ...
        $DB->set_field('extendedquiz_reports', 'capability', 'mod/extendedquiz:grade', array('name' => 'grading'));

        // Mod quiz savepoint reached.
        upgrade_mod_savepoint(true, 2012112901, 'extendedquiz');
    }
