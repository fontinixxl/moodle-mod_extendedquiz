 ...
 * @since     Moodle 2.0
 */
class moodle_extendedquiz_exception extends moodle_exception {
    public function __construct($quizobj, $errorcode, $a = null, $link = '', $debuginfo = null) {
        if (!$link) {
            $link = $quizobj->view_url();
        }
        parent::__construct($errorcode, 'extendedquiz', $link, $a, $debuginfo);
    }
}
...
class extendedquiz {
    ...
    public function __construct($quiz, $cm, $course, $getcontext = true) {
        ...
        $questionids = extendedquiz_questions_in_quiz($this->quiz->questions);
        if ($questionids) {
            $this->questionids = explode(',', extendedquiz_questions_in_quiz($this->quiz->questions));
        } else {
     ...
     * @return extendedquiz the new quiz object
     */
    public static function create($quizid, $userid) {
        global $DB;

        $quiz = extendedquiz_access_manager::load_quiz_and_settings($quizid);
        $course = $DB->get_record('course', array('id' => $quiz->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('extendedquiz', $quiz->id, $course->id, false, MUST_EXIST);

        // Update quiz with override information.
        $quiz = extendedquiz_update_effective_access($quiz, $userid);

        return new extendedquiz($quiz, $cm, $course);
    }
//                                                                                                      ?¿?¿?¿AQUI M'HE QUEDAT :(
    /**
     * Create a {@link quiz_attempt} for an attempt at this quiz.
     * @param object $attemptdata row from the quiz_attempts table.
     * @return extendedquiz_attempt the new quiz_attempt object.
     */
    public function create_attempt_object($attemptdata) {
        return new extendedquiz_attempt($attemptdata, $this->quiz, $this->cm, $this->course);
    }
    ...
    public function preload_questions() {
        if (empty($this->questionids)) {
            throw new moodle_extendedquiz_exception($this, 'noquestions', $this->edit_url());
        }
        $this->questions = question_preload_questions($this->questionids,
                'qqi.grade AS maxmark, qqi.id AS instance',
                '{extendedquiz_q_instances} qqi ON qqi.quiz = :quizid AND q.id = qqi.question',
    ...
    public function get_questions($questionids = null) {
        ...
        foreach ($questionids as $id) {
            if (!array_key_exists($id, $this->questions)) {
                throw new moodle_exception('cannotstartmissingquestion', 'extendedquiz', $this->view_url());
            }
     ...
     * @return extendedquiz_access_manager and instance of the quiz_access_manager class
     *      for this quiz at this time.
     */
    public function get_access_manager($timenow) {
        if (is_null($this->accessmanager)) {
            $this->accessmanager = new extendedquiz_access_manager($this, $timenow,
                    has_capability('mod/extendedquiz:ignoretimelimits', $this->context, null, false));
        }
    ...
    public function view_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/extendedquiz/view.php?id=' . $this->cm->id;
    }
    ...
    public function edit_url() {
        global $CFG;
        return $CFG->wwwroot . '/mod/extendedquiz/edit.php?cmid=' . $this->cm->id;
    }
    ...
    public function attempt_url($attemptid, $page = 0) {
        global $CFG;
        $url = $CFG->wwwroot . '/mod/extendedquiz/attempt.php?attempt=' . $attemptid;
        if ($page) {
            $url .= '&page=' . $page;
        }
        return $url;
    ...
    public function start_attempt_url($page = 0) {
        ...
        return new moodle_url('/mod/extendedquiz/startattempt.php', $params);
    }
    ...
    public function review_url($attemptid) {
        return new moodle_url('/mod/extendedquiz/review.php', array('attempt' => $attemptid));
    }
    ...
    public function summary_url($attemptid) {
        return new moodle_url('/mod/extendedquiz/summary.php', array('attempt' => $attemptid));
    }
    ...
    public function cannot_review_message($when, $short = false) {
        ...
        if ($short) {
         ...
         //canviar la funcio per mod_extendedquiz...
        if ($when == mod_extendedquiz_display_options::DURING ||
                $when == mod_extendedquiz_display_options::IMMEDIATELY_AFTER) {
            return '';
        } else if ($when == mod_extendedquiz_display_options::LATER_WHILE_OPEN && $this->quiz->timeclose &&
                $this->quiz->reviewattempt & mod_extendedquiz_display_options::AFTER_CLOSE) {
    ...
    protected function ensure_question_loaded($id) {
        if (isset($this->questions[$id]->_partiallyloaded)) {
            throw new moodle_extendedquiz_exception($this, 'questionnotloaded', $id);
        }
    }
}
...
class extendedquiz_attempt {
    ...
    protected $attempt;
    // Extendedquiz
    /** @var String intro for an unique attemps with random vars values, if exist it */
    ...
    /** @var mod_extendedquiz_display_options cache for the appropriate review options. */
    ...
    public function __construct($attempt, $quiz, $cm, $course, $loadquestions = true) {
        ...
        $this->quizobj = new extendedquiz($quiz, $cm, $course);
     ...
     * @param array $conditions passed to $DB->get_record('extendedquiz_attempts', $conditions).
     */
    protected static function create_helper($conditions) {
        global $DB;

        $attempt = $DB->get_record('extendedquiz_attempts', $conditions, '*', MUST_EXIST);
        $quiz = extendedquiz_access_manager::load_quiz_and_settings($attempt->quiz);
        $course = $DB->get_record('course', array('id' => $quiz->course), '*', MUST_EXIST);
        $cm = get_coursemodule_from_instance('extendedquiz', $quiz->id, $course->id, false, MUST_EXIST);

        // Update quiz with override information.
        $quiz = extendedquiz_update_effective_access($quiz, $attempt->userid);

        return new extendedquiz_attempt($attempt, $quiz, $cm, $course);
    }
    ...
    public static function state_name($state) {
        return extendedquiz_attempt_state_name($state);
    }
    ...
    private function determine_layout() {
        ...
        $pagelayouts = explode(',0', extendedquiz_clean_layout($this->attempt->layout, true));
    ...
    public function assign_random_values() {
        ...
        if ($vars) {
            foreach ($vars as $var) {
                ...
                $questiontext = str_replace('{$'.$var->varname.'}', implode(', ', $values) , $questiontext);
    ...
    public function is_review_allowed() {
        if (!$this->has_capability('mod/extendedquiz:viewreports')) {
            return false;
        }
    ...
    public function get_overall_feedback($grade) {
        return extendedquiz_feedback_for_grade($grade, $this->get_quiz(),
    ...
    public function check_review_capability() {
        if (!$this->has_capability('mod/extendedquiz:viewreports')) {
            if ($this->get_attempt_state() == mod_extendedquiz_display_options::IMMEDIATELY_AFTER) {
                $this->require_capability('mod/extendedquiz:attempt');
            } else {
                $this->require_capability('mod/extendedquiz:reviewmyattempts');
            }
        }
    }
    ...
    public function can_navigate_to($slot) {
        switch ($this->get_navigation_method()) {
            case EXTENDEDQUIZ_NAVMETHOD_FREE:
                return true;
                break;
            case EXTENDEDQUIZ_NAVMETHOD_SEQ:
                return false;
                break;
        }
        return true;
    ...
    public function get_attempt_state() {
        return extendedquiz_attempt_state($this->get_quiz(), $this->attempt);
    }
    ...
    public function get_display_options($reviewing) {
        if ($reviewing) {
            if (is_null($this->reviewoptions)) {
                $this->reviewoptions = extendedquiz_get_review_options($this->get_quiz(),
                        $this->attempt, $this->quizobj->get_context());
            }
            return $this->reviewoptions;

        } else {
            $options = mod_extendedquiz_display_options::make_from_quiz($this->get_quiz(),
                    mod_extendedquiz_display_options::DURING);
            $options->flags = extendedquiz_get_flag_option($this->attempt, $this->quizobj->get_context());
            return $options;
        }
    }
    ...
    public function get_question_mark($slot) {
        return extendedquiz_format_question_grade($this->get_quiz(), $this->quba->get_question_mark($slot));
    }
    ...
    public function summary_url() {
        return new moodle_url('/mod/extendedquiz/summary.php', array('attempt' => $this->attempt->id));
    }
    ...
    public function processattempt_url() {
        return new moodle_url('/mod/extendedquiz/processattempt.php');
    }
     ...
     * @return extendedquiz_nav_panel_base the requested object.
     */
    public function get_navigation_panel(mod_extendedquiz_renderer $output,
             $panelclass, $page, $showall = false) {
        $panel = new $panelclass($this, $this->get_display_options(true), $page, $showall);

        $bc = new block_contents();
        $bc->attributes['id'] = 'mod_extendedquiz_navblock';                                            //¿?¿?¿?¿?¿¿?¿
    ...
    public function links_to_other_attempts(moodle_url $url) {
        $attempts = extendedquiz_get_user_attempts($this->get_quiz()->id, $this->attempt->userid, 'all');
        if (count($attempts) <= 1) {
            return false;
        }

        $links = new mod_extendedquiz_links_to_other_attempts();
    ...
    public function process_submitted_actions($timestamp, $becomingoverdue = false) {
        ...
        if ($becomingoverdue) {
            ...
            $DB->update_record('extendedquiz_attempts', $this->attempt);
        }

        if (!$this->is_preview() && $this->attempt->state == self::FINISHED) {
            extendedquiz_save_best_grade($this->get_quiz(), $this->get_userid());
        }
    ...
    public function process_finish($timestamp, $processsubmitted) {
        ...
        if (!$this->is_preview()) {
            extendedquiz_save_best_grade($this->get_quiz(), $this->attempt->userid);

            // Trigger event.
            $this->fire_state_transition_event('extendedquiz_attempt_submitted', $timestamp);
    ...
    public function update_timecheckstate($time) {
        ...
        if ($this->attempt->timecheckstate !== $time) {
            ...
            $DB->set_field('extendedquiz_attempts', 'timecheckstate', $time, array('id'=>$this->attempt->id));
        }
    }
    ...
    public function process_going_overdue($timestamp, $studentisonline) {
        ...
        $this->fire_state_transition_event('extendedquiz_attempt_overdue', $timestamp);
    ...
    protected function fire_state_transition_event($event, $timestamp) {
        ...
        if ($event == 'extendedquiz_attempt_submitted') {
    ...
    protected function page_and_question_url($script, $slot, $page, $showall, $thispage) {
        ...
        if ($thispage == $page) {
            ...
            $url = new moodle_url('/mod/extendedquiz/' . $script . '.php' . $fragment,
 ...
 * @since      Moodle 2.1
 */
class extendedquiz_nav_question_button implements renderable {
    public $id;
    public $number;
    public $stateclass;
    public $statestring;
 ...
 * @since      Moodle 2.0
 */
abstract class extendedquiz_nav_panel_base {
    /** @var extendedquiz_attempt */
    ...
    public function __construct(extendedquiz_attempt $attemptobj,
    ...
    public function get_question_buttons() {
        ...
        foreach ($this->attemptobj->get_slots() as $slot) {
            ...
            $button = new extendedquiz_nav_question_button();
    ...
    public function render_before_button_bits(mod_extendedquiz_renderer $output) {
        return '';
    }

    abstract public function render_end_bits(mod_extendedquiz_renderer $output);
 ...
 * @since      Moodle 2.0
 */
class extendedquiz_attempt_nav_panel extends extendedquiz_nav_panel_base {
    ...
    public function render_end_bits(mod_extendedquiz_renderer $output) {
 ...
 * @since      Moodle 2.0
 */
class extendedquiz_review_nav_panel extends extendedquiz_nav_panel_base {
    public function get_question_url($slot) {
