...
$cm = get_coursemodule_from_id('extendedquiz', $id, 0, false, MUST_EXIST);
$course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
$quiz = $DB->get_record('extendedquiz', array('id' => $cm->instance), '*', MUST_EXIST);
require_login($course, false, $cm);

$reportlist = extendedquiz_report_list(context_module::instance($cm->id));
if (empty($reportlist) || $userid == $USER->id) {
    ...
    // but it is possible.)
    redirect(new moodle_url('/mod/extendedquiz/view.php', array('id' => $cm->id)));
}
...
if ($userid) {
    ...
    $attempts = extendedquiz_get_user_attempts($quiz->id, $userid, 'finished');
    $attempt = null;
    ...
    if ($attempt) {
        $attemptobj = new extendedquiz_attempt($attempt, $quiz, $cm, $course, false);
...
redirect(new moodle_url('/mod/extendedquiz/report.php', array(
