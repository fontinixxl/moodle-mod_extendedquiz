 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class extendedquiz_override_form extends moodleform {
    ...
    protected function definition() {
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/extendedquiz/overrides.php', array('cmid'=>$cm->id));
                    print_error('groupsnone', 'extendedquiz', $link);
                }
            ...
            if ($this->userid) {
                ...
                if (!empty($CFG->enablegroupmembersonly) && $cm->groupmembersonly) {
                    ...
                    if (!empty($groups)) {
                        $users = get_users_by_capability($this->context, 'mod/extendedquiz:attempt',
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/extendedquiz/overrides.php', array('cmid'=>$cm->id));
                    print_error('usersnone', 'extendedquiz', $link);
                }
