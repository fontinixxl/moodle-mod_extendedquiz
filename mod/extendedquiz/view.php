...
if ($id) {
    if (!$cm = get_coursemodule_from_id('extendedquiz', $id)) {
    ...
    if (!$quiz = $DB->get_record('extendedquiz', array('id' => $q))) {
...
require_capability('mod/extendedquiz:view', $context);

// Cache some other capabilities we use several times.
$canattempt = has_capability('mod/extendedquiz:attempt', $context);
$canreviewmine = has_capability('mod/extendedquiz:reviewmyattempts', $context);
$canpreview = has_capability('mod/extendedquiz:preview', $context);

// Create an object to manage all the other (non-roles) access rules.
$timenow = time();
$quizobj = extendedquiz::create($cm->instance, $USER->id);
$accessmanager = new extendedquiz_access_manager($quizobj, $timenow,
        has_capability('mod/extendedquiz:ignoretimelimits', $context, null, false));
$quiz = $quizobj->get_quiz();

// Log this request.
add_to_log($course->id, 'extendedquiz', 'view', 'view.php?id=' . $cm->id, $quiz->id, $cm->id);

$completion = new completion_info($course);
$completion->set_module_viewed($cm);

// Initialize $PAGE, compute blocks.
$PAGE->set_url('/mod/extendedquiz/view.php', array('id' => $cm->id));

// Create view object which collects all the information the renderer will need.
$viewobj = new mod_extendedquiz_view_object();
$viewobj->accessmanager = $accessmanager;
$viewobj->canreviewmine = $canreviewmine;

// Get this user's attempts.
$attempts = extendedquiz_get_user_attempts($quiz->id, $USER->id, 'finished', true);
$lastfinishedattempt = end($attempts);
$unfinished = false;
if ($unfinishedattempt = extendedquiz_get_user_attempt_unfinished($quiz->id, $USER->id)) {
    ...
    $unfinished = $unfinishedattempt->state == extendedquiz_attempt::IN_PROGRESS ||
            $unfinishedattempt->state == extendedquiz_attempt::OVERDUE;
    if (!$unfinished) {
...
foreach ($attempts as $attempt) {
    $viewobj->attemptobjs[] = new extendedquiz_attempt($attempt, $quiz, $cm, $course, false);
}

// Work out the final grade, checking whether it was overridden in the gradebook.
if (!$canpreview) {
    $mygrade = extendedquiz_get_best_grade($quiz, $USER->id);
    ...
    $mygrade = extendedquiz_rescale_grade($lastfinishedattempt->sumgrades, $quiz, false);
} else {
    $mygrade = null;
}
...
$grading_info = grade_get_grades($course->id, 'mod', 'extendedquiz', $quiz->id, $USER->id);
...
if ($attempts) {
    ...
    list($someoptions, $alloptions) = extendedquiz_get_combined_reviewoptions($quiz, $attempts, $context);

    $viewobj->attemptcolumn  = $quiz->attempts != 1;

    $viewobj->gradecolumn    = $someoptions->marks >= question_display_options::MARK_AND_MAX &&
            extendedquiz_has_grades($quiz);
    $viewobj->markcolumn     = $viewobj->gradecolumn && ($quiz->grade != $quiz->sumgrades);
    $viewobj->overallstats   = $lastfinishedattempt && $alloptions->marks >= question_display_options::MARK_AND_MAX;

    $viewobj->feedbackcolumn = extendedquiz_has_feedback($quiz) && $alloptions->overallfeedback;
}
...
$viewobj->canedit = has_capability('mod/extendedquiz:manage', $context);
$viewobj->editurl = new moodle_url('/mod/extendedquiz/edit.php', array('cmid' => $cm->id));
...
if ($quiz->attempts != 1) {
    $viewobj->infomessages[] = get_string('gradingmethod', 'quiz',
            extendedquiz_get_grading_option_name($quiz->grademethod));
}

// Determine wheter a start attempt button should be displayed.
$viewobj->quizhasquestions = (bool) extendedquiz_clean_layout($quiz->questions, true);
