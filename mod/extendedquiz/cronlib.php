 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_extendedquiz_overdue_attempt_updater {
        ...
        foreach ($attemptstoprocess as $attempt) {
            try {
                ...
                if (!$quiz || $attempt->quiz != $quiz->id) {
                    $quiz = $DB->get_record('extendedquiz', array('id' => $attempt->quiz), '*', MUST_EXIST);
                    $cm = get_coursemodule_from_instance('extendedquiz', $attempt->quiz);
                    $quizcount += 1;
                }
                ...
                $attemptobj = new extendedquiz_attempt($attempt, $quizforuser, $cm, $course);
