...
M.mod_extendedquiz = M.mod_extendedquiz || {};

M.mod_extendedquiz.init_attempt_form = function(Y) {
    M.core_question_engine.init_form(Y, '#responseform');
    Y.on('submit', M.mod_extendedquiz.timer.stop, '#responseform');
    M.core_formchangechecker.init({formid: 'responseform'});
};

M.mod_extendedquiz.init_review_form = function(Y) {
    M.core_question_engine.init_form(Y, '.questionflagsaveform');
    Y.on('submit', function(e) { e.halt(); }, '.questionflagsaveform');
};

M.mod_extendedquiz.init_comment_popup = function(Y) {
...
M.mod_extendedquiz.timer = {
    ...
    init: function(Y, start, preview) {
        M.mod_extendedquiz.timer.Y = Y;
        M.mod_extendedquiz.timer.endtime = M.pageloadstarttime.getTime() + start*1000;
        M.mod_extendedquiz.timer.preview = preview;
        M.mod_extendedquiz.timer.update();
     ...
     */
    stop: function(e) {
        if (M.mod_extendedquiz.timer.timeoutid) {
            clearTimeout(M.mod_extendedquiz.timer.timeoutid);
        }
    },
    ...
    update: function() {
        var Y = M.mod_extendedquiz.timer.Y;
        var secondsleft = Math.floor((M.mod_extendedquiz.timer.endtime - new Date().getTime())/1000);
        
        // If time has expired, set the hidden form field that says time has expired and submit
        if (secondsleft < 0) {
            M.mod_extendedquiz.timer.stop(null);
        ...
        Y.one('#quiz-time-left').setContent(hours + ':' +
                M.mod_extendedquiz.timer.two_digit(minutes) + ':' +
                M.mod_extendedquiz.timer.two_digit(seconds));

        // Arrange for this method to be called again soon.
        M.mod_extendedquiz.timer.timeoutid = setTimeout(M.mod_extendedquiz.timer.update, 100);
    }
};

M.mod_extendedquiz.nav = M.mod_extendedquiz.nav || {};

M.mod_extendedquiz.nav.update_flag_state = function(attemptid, questionid, newstate) {
    var Y = M.mod_extendedquiz.nav.Y;
...
M.mod_extendedquiz.nav.init = function(Y) {
    M.mod_extendedquiz.nav.Y = Y;
    ...
    if (M.core_question_flags) {
        M.core_question_flags.add_listener(M.mod_extendedquiz.nav.update_flag_state);
    }
};

M.mod_extendedquiz.secure_window = {
    init: function(Y) {
        if (window.location.href.substring(0, 4) == 'file') {
            window.location = 'about:blank';
        }
        Y.delegate('contextmenu', M.mod_extendedquiz.secure_window.prevent, document, '*');
        Y.delegate('mousedown',   M.mod_extendedquiz.secure_window.prevent_mouse, document, '*');
        Y.delegate('mouseup',     M.mod_extendedquiz.secure_window.prevent_mouse, document, '*');
        Y.delegate('dragstart',   M.mod_extendedquiz.secure_window.prevent, document, '*');
        Y.delegate('selectstart', M.mod_extendedquiz.secure_window.prevent, document, '*');
        Y.delegate('cut',         M.mod_extendedquiz.secure_window.prevent, document, '*');
        Y.delegate('copy',        M.mod_extendedquiz.secure_window.prevent, document, '*');
        Y.delegate('paste',       M.mod_extendedquiz.secure_window.prevent, document, '*');
        M.mod_extendedquiz.secure_window.clear_status;
        ...
        Y.on('key', M.mod_extendedquiz.secure_window.prevent, '*', 'press:67,86,88+ctrl');
        Y.on('key', M.mod_extendedquiz.secure_window.prevent, '*', 'up:67,86,88+ctrl');
        Y.on('key', M.mod_extendedquiz.secure_window.prevent, '*', 'down:67,86,88+ctrl');
        Y.on('key', M.mod_extendedquiz.secure_window.prevent, '*', 'press:67,86,88+meta');
        Y.on('key', M.mod_extendedquiz.secure_window.prevent, '*', 'up:67,86,88+meta');
        Y.on('key', M.mod_extendedquiz.secure_window.prevent, '*', 'down:67,86,88+meta');
    },

    clear_status: function() {
        window.status = '';
        setTimeout(M.mod_extendedquiz.secure_window.clear_status, 10);
    },
    ...
    init_close_button: function(Y, url) {
        Y.on('click', function(e) {
            M.mod_extendedquiz.secure_window.close(url, 0)
        }, '#secureclosebutton');
    },
