 ...
 */
function extendedquiz_report_index_by_keys($datum, $keys, $keysunique = true) {
    if (!$datum) {
        return array();
    }
    ...
    if ($keys) {
        foreach ($datumkeyed as $datakey => $datakeyed) {
            $datumkeyed[$datakey] = extendedquiz_report_index_by_keys($datakeyed, $keys, $keysunique);
        }
    }
    return $datumkeyed;
}

function extendedquiz_report_unindex($datum) {
    if (!$datum) {
        return $datum;
    }
    $datumunkeyed = array();
    foreach ($datum as $value) {
        if (is_array($value)) {
            $datumunkeyed = array_merge($datumunkeyed, extendedquiz_report_unindex($value));
        } else {
            $datumunkeyed[] = $value;
        }
    }
 ...
 */
function extendedquiz_report_get_significant_questions($quiz) {
    global $DB;
 ...
 */
function extendedquiz_report_can_filter_only_graded($quiz) {
    return $quiz->attempts != 1 && $quiz->grademethod != EXTENDEDQUIZ_GRADEAVERAGE;
}
 ...
 */
function extendedquiz_report_qm_filter_select($quiz, $quizattemptsalias = 'quiza') {
    if ($quiz->attempts == 1) {
    ...
    switch ($quiz->grademethod) {
        case EXTENDEDQUIZ_GRADEHIGHEST :
            return "$quizattemptsalias.id = (
                    SELECT MIN(qa2.id)
                    FROM {extendedquiz_attempts} qa2
                    WHERE qa2.quiz = $quizattemptsalias.quiz AND
                        ...
                        COALESCE(qa2.sumgrades, 0) = (
                            ...
                            FROM {extendedquiz_attempts} qa3
 ...
 */
function extendedquiz_report_grade_bands($bandwidth, $bands, $quizid, $userids = array()) {
    global $DB;
    if (!is_int($bands)) {
        debugging('$bands passed to extendedquiz_report_grade_bands must be an integer. (' .
...
function extendedquiz_report_highlighting_grading_method($quiz, $qmsubselect, $qmfilter) {
    if ($quiz->attempts == 1) {
        return '<p>' . get_string('onlyoneattemptallowed', 'extendedquiz_overview') . '</p>';

    } else if (!$qmsubselect) {
        return '<p>' . get_string('allattemptscontributetograde', 'extendedquiz_overview') . '</p>';

    } else if ($qmfilter) {
        return '<p>' . get_string('showinggraded', 'extendedquiz_overview') . '</p>';

    } else {
        return '<p>' . get_string('showinggradedandungraded', 'extendedquiz_overview',
                '<span class="gradedattempt">' . extendedquiz_get_grading_option_name($quiz->grademethod) .
                '</span>') . '</p>';
    }
}
 ...
 */
function extendedquiz_report_feedback_for_grade($grade, $quizid, $context) {
    global $DB;

    static $feedbackcache = array();

    if (!isset($feedbackcache[$quizid])) {
        $feedbackcache[$quizid] = $DB->get_records('extendedquiz_feedback', array('quizid' => $quizid));
    }
    ...
    $feedbacktext = file_rewrite_pluginfile_urls($feedbacktext, 'pluginfile.php',
            $context->id, 'mod_extendedquiz', 'feedback', $feedbackid);
 ...
 */
function extendedquiz_report_scale_summarks_as_percentage($rawmark, $quiz, $round = true) {
    if ($quiz->sumgrades == 0) {
        return '';
    }
    ...
    if ($round) {
        $mark = extendedquiz_format_grade($quiz, $mark);
    }
    return $mark . '%';
}
 ...
 */
function extendedquiz_report_list($context) {
    global $DB;
    ...
    $reports = $DB->get_records('extendedquiz_reports', null, 'displayorder DESC', 'name, capability');
    ...
    foreach ($reportcaps as $name => $capability) {
        if (empty($capability)) {
            $capability = 'mod/extendedquiz:viewreports';
        }
 ...
 */
function extendedquiz_report_download_filename($report, $courseshortname, $quizname) {
 ...
 */
function extendedquiz_report_default_report($context) {
    $reports = extendedquiz_report_list($context);
 ...
 */
function extendedquiz_no_questions_message($quiz, $cm, $context) {
    global $OUTPUT;

    $output = '';
    $output .= $OUTPUT->notification(get_string('noquestions', 'quiz'));
    if (has_capability('mod/extendedquiz:manage', $context)) {
        $output .= $OUTPUT->single_button(new moodle_url('/mod/extendedquiz/edit.php',
 ...
 * @return bool
 */
function extendedquiz_report_should_show_grades($quiz, context $context) {
    if ($quiz->timeclose && time() > $quiz->timeclose) {
        $when = mod_extendedquiz_display_options::AFTER_CLOSE;
    } else {
        $when = mod_extendedquiz_display_options::LATER_WHILE_OPEN;
    }
    $reviewoptions = mod_extendedquiz_display_options::make_from_quiz($quiz, $when);

    return extendedquiz_has_grades($quiz) &&
