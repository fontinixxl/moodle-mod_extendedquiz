...
/**
 * Base class for the options that control what is visible in an {@link extendedquiz_attempts_report}.
 *
 * @copyright 2012 The Open University
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_extendedquiz_attempts_report_options {
    ...
    protected static $statefields = array(
        'stateinprogress' => extendedquiz_attempt::IN_PROGRESS,
        'stateoverdue'    => extendedquiz_attempt::OVERDUE,
        'statefinished'   => extendedquiz_attempt::FINISHED,
        'stateabandoned'  => extendedquiz_attempt::ABANDONED,
    );
     ...
     */
    public $attempts = extendedquiz_attempts_report::ENROLLED_WITH;
     ...
     *      no restriction.
     */
    public $states = array(extendedquiz_attempt::IN_PROGRESS, extendedquiz_attempt::OVERDUE,
            extendedquiz_attempt::FINISHED, extendedquiz_attempt::ABANDONED);
    ...
    public $pagesize = extendedquiz_attempts_report::DEFAULT_PAGE_SIZE;
    ...
    public function __construct($mode, $quiz, $cm, $course) {
        ...
        $this->usercanseegrades = extendedquiz_report_should_show_grades($quiz, context_module::instance($cm->id));
    }
    ...
    public function get_url() {
        return new moodle_url('/mod/extendedquiz/report.php', $this->get_url_params());
    }
    ...
    public function setup_from_user_preferences() {
        $this->pagesize = get_user_preferences('extendedquiz_report_pagesize', $this->pagesize);
    }
    ...
    public function update_user_preferences() {
        set_user_preference('extendedquiz_report_pagesize', $this->pagesize);
    }
    ...
    public function resolve_dependencies() {
        if ($this->group) {
            ...
            if ($this->attempts === null || $this->attempts == extendedquiz_attempts_report::ALL_WITH) {
                $this->attempts = extendedquiz_attempts_report::ENROLLED_WITH;
            }

        } else if (!$this->group && $this->course->id == SITEID) {
            // Force report on front page to show all, unless a group is selected.
            $this->attempts = extendedquiz_attempts_report::ALL_WITH;

        } else if (!in_array($this->attempts, array(extendedquiz_attempts_report::ALL_WITH, extendedquiz_attempts_report::ENROLLED_WITH,
                extendedquiz_attempts_report::ENROLLED_WITHOUT, extendedquiz_attempts_report::ENROLLED_ALL))) {
            $this->attempts = extendedquiz_attempts_report::ENROLLED_WITH;
        }
        ...
        if (!extendedquiz_report_can_filter_only_graded($this->quiz)) {
        ...
        if ($this->attempts == extendedquiz_attempts_report::ENROLLED_WITHOUT) {
            $this->states = null;
            $this->onlygraded = false;
        }

        if ($this->onlygraded) {
            $this->states = array(extendedquiz_attempt::FINISHED);
        }

        if ($this->pagesize < 1) {
            $this->pagesize = extendedquiz_attempts_report::DEFAULT_PAGE_SIZE;
        }
    }
}
