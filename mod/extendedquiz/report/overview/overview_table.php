...
require_once($CFG->dirroot . '/mod/extendedquiz/report/attemptsreport_table.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class extendedquiz_overview_table extends extendedquiz_attempts_report_table {
     ...
     * @param extendedquiz_overview_options $options
    ...
    public function __construct($quiz, $context, $qmsubselect,
            extendedquiz_overview_options $options, $groupstudents, $students, $questions, $reporturl) {
        parent::__construct('mod-extendedquiz-report-overview-report', $quiz , $context,
    ...
    protected function add_average_row($label, $users) {
        ...
        $record = $DB->get_record_sql("
                SELECT AVG(quiza.sumgrades) AS grade, COUNT(quiza.sumgrades) AS numaveraged
                  FROM $from
                 WHERE $where", $params);
        $record->grade = extendedquiz_rescale_grade($record->grade, $this->quiz, false);
        ...
        $averagerow = array(
            ...
            'feedbacktext'=> strip_tags(extendedquiz_report_feedback_for_grade(
    ...
    protected function format_average_grade_for_questions($gradeaverages) {
        ...
        foreach ($this->questions as $question) {
            if (isset($gradeaverages[$question->slot]) && $question->maxmark > 0) {
                ...
                $record->grade = extendedquiz_rescale_grade(
    ...
    protected function format_average($record, $question = false) {
        if (is_null($record->grade)) {
            ...
            $average = extendedquiz_format_question_grade($this->quiz, $record->grade);
        } else {
            $average = extendedquiz_format_grade($this->quiz, $record->grade);
        }
    ...
    protected function submit_buttons() {
        if (has_capability('mod/extendedquiz:regrade', $this->context)) {
            echo '<input type="submit" name="regrade" value="' .
                    get_string('regradeselected', 'extendedquiz_overview') . '"/>';
        }
    ...
    public function col_sumgrades($attempt) {
        ...
        $grade = extendedquiz_rescale_grade($attempt->sumgrades, $this->quiz);
        if ($this->is_downloading()) {
            return $grade;
        }
        ...
        if (isset($this->regradedqs[$attempt->usageid])) {
            ...
            $newsumgrade = extendedquiz_rescale_grade($newsumgrade, $this->quiz);
            $oldsumgrade = extendedquiz_rescale_grade($oldsumgrade, $this->quiz);
            $grade = html_writer::tag('del', $oldsumgrade) . '/' .
                    html_writer::empty_tag('br') . $newsumgrade;
        }
        return html_writer::link(new moodle_url('/mod/extendedquiz/review.php',
     ...
     * mod/extendedquiz/report/overview/report.php to see what fields are present,
    ...
    public function other_cols($colname, $attempt) {
        ...
        if ($question->maxmark == 0) {
            ...
            $grade = extendedquiz_rescale_grade(
        ...
        if (isset($this->regradedqs[$attempt->usageid][$slot])) {
            $gradefromdb = $grade;
            $newgrade = extendedquiz_rescale_grade(
                    $this->regradedqs[$attempt->usageid][$slot]->newfraction * $question->maxmark,
                    $this->quiz, 'question');
            $oldgrade = extendedquiz_rescale_grade(
    ...
    public function col_regraded($attempt) {
        if ($attempt->regraded == '') {
            ...
            return get_string('needed', 'extendedquiz_overview');
        } else if ($attempt->regraded == 1) {
            return get_string('done', 'extendedquiz_overview');
        }
    }
    ...
    public function query_db($pagesize, $useinitialsbar = true) {
        ...
        if ($this->options->slotmarks && has_capability('mod/extendedquiz:regrade', $this->context)) {
    ...
    protected function get_regraded_questions() {
        ...
        $regradedqs = $DB->get_records_select('extendedquiz_overw_regrades',
                'questionusageid ' . $qubaids->usage_id_in(), $qubaids->usage_id_in_params());
        return extendedquiz_report_index_by_keys($regradedqs, array('questionusageid', 'slot'));
    }
}
