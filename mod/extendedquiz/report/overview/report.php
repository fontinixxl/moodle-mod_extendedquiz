...
require_once($CFG->dirroot . '/mod/extendedquiz/report/attemptsreport.php');
require_once($CFG->dirroot . '/mod/extendedquiz/report/overview/overview_options.php');
require_once($CFG->dirroot . '/mod/extendedquiz/report/overview/overview_form.php');
require_once($CFG->dirroot . '/mod/extendedquiz/report/overview/overview_table.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class extendedquiz_overview_report extends extendedquiz_attempts_report {

    public function display($quiz, $cm, $course) {
        global $CFG, $DB, $OUTPUT, $PAGE;

        list($currentgroup, $students, $groupstudents, $allowed) =
                $this->init('overview', 'extendedquiz_overview_settings_form', $quiz, $cm, $course);
        $options = new extendedquiz_overview_options('overview', $quiz, $cm, $course);
        ...
        $table = new extendedquiz_overview_table($quiz, $this->context, $this->qmsubselect,
                $options, $groupstudents, $students, $questions, $options->get_url());
        $filename = extendedquiz_report_download_filename(get_string('overviewfilename', 'extendedquiz_overview'),
        ...
        if (!$table->is_downloading()) {
            ...
            if ($strattemptnum = extendedquiz_num_attempt_summary($quiz, $cm, true, $currentgroup)) {
                echo '<div class="quizattemptcounts">' . $strattemptnum . '</div>';
            }
        }

        $hasquestions = extendedquiz_questions_in_quiz($quiz->questions);
        if (!$table->is_downloading()) {
            if (!$hasquestions) {
                echo extendedquiz_no_questions_message($quiz, $cm, $this->context);
            } else if (!$students) {
        ...
        if ($hasquestions && ($hasstudents || $options->attempts == self::ALL_WITH)) {
            ...
            $fields .= ", COALESCE((
                                SELECT MAX(qqr.regraded)
                                  FROM {extendedquiz_overw_regrades} qqr
            ...
            if (!$table->is_downloading()) {
                ...
                if (has_capability('mod/extendedquiz:regrade', $this->context)) {
                    ...
                    if ($currentgroup) {
                        ...
                        $regradealldrydolabel =
                                get_string('regradealldrydogroup', 'extendedquiz_overview', $a);
                        $regradealldrylabel =
                                get_string('regradealldrygroup', 'extendedquiz_overview', $a);
                        $regradealllabel =
                                get_string('regradeallgroup', 'extendedquiz_overview', $a);
                    } else {
                        $regradealldrydolabel =
                                get_string('regradealldrydo', 'extendedquiz_overview', $regradesneeded);
                        $regradealldrylabel =
                                get_string('regradealldry', 'extendedquiz_overview');
                        $regradealllabel =
                                get_string('regradeall', 'extendedquiz_overview');
                    }
                ...
                if ($strattempthighlight = extendedquiz_report_highlighting_grading_method(
            ...
            if (!$table->is_downloading() && has_capability('mod/extendedquiz:regrade', $this->context) &&
                    $this->has_regraded_questions($from, $where, $params)) {
                $columns[] = 'regraded';
                $headers[] = get_string('regrade', 'extendedquiz_overview');
            }
            ...
            if ($options->slotmarks) {
                foreach ($questions as $slot => $question) {
                    ...
                    $header .= '/' . extendedquiz_rescale_grade($question->maxmark, $quiz, 'question');
                    $headers[] = $header;
                }
            }
        ...
        if (!$table->is_downloading() && $options->usercanseegrades) {
            ...
            if ($currentgroup && $groupstudents) {
                ...
                $params[] = $quiz->id;
                if ($DB->record_exists_select('extendedquiz_grades', "userid $usql AND quiz = ?",
                        $params)) {
                    $imageurl = new moodle_url('/mod/extendedquiz/report/overview/overviewgraph.php',
                            array('id' => $quiz->id, 'groupid' => $currentgroup));
                    $graphname = get_string('overviewreportgraphgroup', 'extendedquiz_overview',
                            groups_get_group_name($currentgroup));
                    echo $output->graph($imageurl, $graphname);
                }
            }

            if ($DB->record_exists('extendedquiz_grades', array('quiz'=> $quiz->id))) {
                $imageurl = new moodle_url('/mod/extendedquiz/report/overview/overviewgraph.php',
                        array('id' => $quiz->id));
                $graphname = get_string('overviewreportgraph', 'extendedquiz_overview');
                echo $output->graph($imageurl, $graphname);
            }
        }
        return true;
    ...
    protected function start_regrade($quiz, $cm) {
        global $OUTPUT, $PAGE;
        require_capability('mod/extendedquiz:regrade', $this->context);
    ...
    protected function finish_regrade($nexturl) {
        global $OUTPUT, $PAGE;
        echo $OUTPUT->heading(get_string('regradecomplete', 'extendedquiz_overview'));
    ...
    protected function regrade_attempt($attempt, $dryrun = false, $slots = null) {
        ...
        $finished = $attempt->state == extendedquiz_attempt::FINISHED;
        foreach ($slots as $slot) {
            ...
            if (abs($qqr->oldfraction - $qqr->newfraction) > 1e-7) {
                ...
                $DB->insert_record('extendedquiz_overw_regrades', $qqr, false);
            }
        }
    ...
    protected function regrade_attempts($quiz, $dryrun = false,
        ...
        $attempts = $DB->get_records_select('extendedquiz_attempts', $where, $params);
        if (!$attempts) {
            return;
        }
        ...
        $progressbar = new progress_bar('extendedquiz_overview_regrade', 500, true);          //?¿¿?¿?
        $a = array(
        ...
        foreach ($attempts as $attempt) {
            ...
            $progressbar->update($a['done'], $a['count'],
                    get_string('regradingattemptxofy', 'extendedquiz_overview', $a));
        }
    ...
    protected function regrade_attempts_needing_it($quiz, $groupstudents) {
        ...
        $toregrade = $DB->get_records_sql("
                ...
                JOIN {extendedquiz_overw_regrades} qqr ON qqr.questionusageid = quiza.uniqueid
                WHERE $where", $params);
        ...
        $attempts = $DB->get_records_list('extendedquiz_attempts', 'uniqueid',
    ...
    protected function count_question_attempts_needing_regrade($quiz, $groupstudents) {
        ...
        $sql = "SELECT COUNT(DISTINCT quiza.id)
                FROM {extendedquiz_attempts} quiza
                JOIN {extendedquiz_overw_regrades} qqr ON quiza.uniqueid = qqr.questionusageid
                WHERE
                    $usertest
                    quiza.quiz = ? AND
                    quiza.preview = 0 AND
    ...
    protected function has_regraded_questions($from, $where, $params) {
        ...
        return $DB->record_exists_select('extendedquiz_overw_regrades',
    ...
    protected function clear_regrade_table($quiz, $groupstudents) {
        ...
        $params[] = $quiz->id;
        $DB->delete_records_select('extendedquiz_overw_regrades',
                "questionusageid IN (
                    SELECT uniqueid
                    FROM {extendedquiz_attempts}
                    WHERE $where quiz = ?
                )", $params);
    }
    ...
    protected function update_overall_grades($quiz) {
        extendedquiz_update_all_attempt_sumgrades($quiz);
