...
$quiz = $DB->get_record('extendedquiz', array('id' => $quizid));
$course = $DB->get_record('course', array('id' => $quiz->course));
$cm = get_coursemodule_from_instance('extendedquiz', $quizid);

require_login($course, false, $cm);
$modcontext = context_module::instance($cm->id);
require_capability('mod/extendedquiz:viewreports', $modcontext);
...
$bandlabels = array();
for ($i = 1; $i <= $bands; $i++) {
    $bandlabels[] = extendedquiz_format_grade($quiz, ($i - 1) * $bandwidth) . ' - ' .
            extendedquiz_format_grade($quiz, $i * $bandwidth);
}
...
$line->y_data['allusers'] = extendedquiz_report_grade_bands($bandwidth, $bands, $quizid, $groupusers);
