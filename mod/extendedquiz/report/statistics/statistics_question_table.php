 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class extendedquiz_statistics_question_table extends flexible_table {
    ...
    public function __construct($qid) {
        parent::__construct('mod-extendedquiz-report-statistics-question-table' . $qid);
    }
    ...
    public function question_setup($reporturl, $questiondata,
            extendedquiz_statistics_response_analyser $responesstats) {
        ...
        if ($responesstats->has_subparts()) {
            $columns[] = 'part';
            $headers[] = get_string('partofquestion', 'extendedquiz_statistics');
        }

        if ($responesstats->has_response_classes()) {
            $columns[] = 'responseclass';
            $headers[] = get_string('modelresponse', 'extendedquiz_statistics');

            if ($responesstats->has_actual_responses()) {
                $columns[] = 'response';
                $headers[] = get_string('actualresponse', 'extendedquiz_statistics');
            }

        } else {
            $columns[] = 'response';
            $headers[] = get_string('response', 'extendedquiz_statistics');
        }

        $columns[] = 'fraction';
        $headers[] = get_string('optiongrade', 'extendedquiz_statistics');

        $columns[] = 'count';
        $headers[] = get_string('count', 'extendedquiz_statistics');

        $columns[] = 'frequency';
        $headers[] = get_string('frequency', 'extendedquiz_statistics');
