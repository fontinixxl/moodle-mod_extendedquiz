 ...
 *
 * @package   extendedquiz_statistics
...
require_once($CFG->dirroot . '/mod/extendedquiz/report/statistics/statistics_form.php');
require_once($CFG->dirroot . '/mod/extendedquiz/report/statistics/statistics_table.php');
require_once($CFG->dirroot . '/mod/extendedquiz/report/statistics/statistics_question_table.php');
require_once($CFG->dirroot . '/mod/extendedquiz/report/statistics/qstats.php');
require_once($CFG->dirroot . '/mod/extendedquiz/report/statistics/responseanalysis.php');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class extendedquiz_statistics_report extends extendedquiz_default_report {
    ...
    public function display($quiz, $cm, $course) {
        ...
        $reporturl = new moodle_url('/mod/extendedquiz/report.php', $pageoptions);

        $mform = new extendedquiz_statistics_settings_form($reporturl);
        if ($fromform = $mform->get_data()) {
            $useallattempts = $fromform->useallattempts;
            if ($fromform->useallattempts) {
                set_user_preference('extendedquiz_report_statistics_useallattempts',
                        $fromform->useallattempts);
            } else {
                unset_user_preference('extendedquiz_report_statistics_useallattempts');
            }

        } else {
            $useallattempts = get_user_preferences('extendedquiz_report_statistics_useallattempts', 0);
        }
        ...
        if (empty($currentgroup)) {
            ...
            $groupstudents = get_users_by_capability($this->context,
                    array('mod/extendedquiz:reviewmyattempts', 'mod/extendedquiz:attempt'),
                    '', '', '', '', $currentgroup, '', false);
            if (!$groupstudents) {
                $nostudentsingroup = true;
            }
        ...
        $this->table = new extendedquiz_statistics_table();
        if ($everything) {
            $report = get_string('completestatsfilename', 'extendedquiz_statistics');
        } else {
            $report = get_string('questionstatsfilename', 'extendedquiz_statistics');
        }
        $courseshortname = format_string($course->shortname, true,
                array('context' => context_course::instance($course->id)));
        $filename = extendedquiz_report_download_filename($report, $courseshortname, $quiz->name);
        $this->table->is_downloading($download, $filename,
                get_string('quizstructureanalysis', 'extendedquiz_statistics'));

        // Load the questions.
        $questions = extendedquiz_report_get_significant_questions($quiz);
        $questionids = array();
        ...
        if (!$this->table->is_downloading()) {
            ...
            if (groups_get_activity_groupmode($cm)) {
                ...
                if ($currentgroup && !$groupstudents) {
                    $OUTPUT->notification(get_string('nostudentsingroup', 'extendedquiz_statistics'));
                }
            }

            if (!extendedquiz_questions_in_quiz($quiz->questions)) {
        ...
        if ($everything) { // Implies is downloading.
            ...
            echo $OUTPUT->box('<a href="' . $reporturl->out() . '">' .
                    get_string('backtoquizreport', 'extendedquiz_statistics') . '</a>',
            ...
            echo $OUTPUT->heading(get_string('quizinformation', 'extendedquiz_statistics'));
            ...
            if ($s) {
                echo $OUTPUT->heading(get_string('quizstructureanalysis', 'extendedquiz_statistics'));
                $this->output_quiz_structure_analysis_table($s, $questions, $subquestions);
                $this->output_statistics_graph($quizstats->id, $s);
            }
        }
    ...
    protected function output_individual_question_data($quiz, $question) {
        ...
        $questioninfotable->data[] = array(get_string('modulename', 'extendedquiz'), $quiz->name);
        $questioninfotable->data[] = array(get_string('questionname', 'extendedquiz_statistics'),
                $question->name.'&nbsp;'.$datumfromtable['actions']);
        $questioninfotable->data[] = array(get_string('questiontype', 'extendedquiz_statistics'),
                $datumfromtable['icon'] . '&nbsp;' .
                question_bank::get_qtype($question->qtype, false)->menu_name() . '&nbsp;' .
                $datumfromtable['icon']);
        $questioninfotable->data[] = array(get_string('positions', 'extendedquiz_statistics'),
        ...
        $labels = array(
            's' => get_string('attempts', 'extendedquiz_statistics'),
            'facility' => get_string('facility', 'extendedquiz_statistics'),
            'sd' => get_string('standarddeviationq', 'extendedquiz_statistics'),
            'random_guess_score' => get_string('random_guess_score', 'extendedquiz_statistics'),
            'intended_weight' => get_string('intended_weight', 'extendedquiz_statistics'),
            'effective_weight' => get_string('effective_weight', 'extendedquiz_statistics'),
            'discrimination_index' => get_string('discrimination_index', 'extendedquiz_statistics'),
            'discriminative_efficiency' =>
                                get_string('discriminative_efficiency', 'extendedquiz_statistics')
        );
        ...
        echo $OUTPUT->heading(get_string('questioninformation', 'extendedquiz_statistics'));
        echo html_writer::table($questioninfotable);
        echo $this->render_question_text($question);
        echo $OUTPUT->heading(get_string('questionstatistics', 'extendedquiz_statistics'));
    ...
    protected function render_question_text($question) {
        ...
        $text = question_rewrite_questiontext_preview_urls($question->questiontext,
                $this->context->id, 'extendedquiz_statistics', $question->id);
    ...
    protected function output_individual_question_response_analysis($question,
        ...
        $qtable = new extendedquiz_statistics_question_table($question->id);
        $exportclass = $this->table->export_class_instance();
        $qtable->export_class_instance($exportclass);
        if (!$this->table->is_downloading()) {
            // Output an appropriate title.
            echo $OUTPUT->heading(get_string('analysisofresponses', 'extendedquiz_statistics'));
            ...
            if ($this->table->is_downloading() == 'xhtml') {
                $questiontabletitle = get_string('analysisofresponsesfor',
                        'extendedquiz_statistics', $questiontabletitle);
            }
        ...
        $responesstats = new extendedquiz_statistics_response_analyser($question);
    ...
    protected function get_formatted_quiz_info_data($course, $cm, $quiz, $quizstats) {
        ...
        $quizinfo = array();
        $quizinfo[get_string('quizname', 'extendedquiz_statistics')] = format_string($quiz->name);
        $quizinfo[get_string('coursename', 'extendedquiz_statistics')] = format_string($course->fullname);
        if ($cm->idnumber) {
            $quizinfo[get_string('idnumbermod')] = $cm->idnumber;
        }
        if ($quiz->timeopen) {
            $quizinfo[get_string('quizopen', 'extendedquiz')] = userdate($quiz->timeopen);
        }
        if ($quiz->timeclose) {
            $quizinfo[get_string('quizclose', 'extendedquiz')] = userdate($quiz->timeclose);
        }
        if ($quiz->timeopen && $quiz->timeclose) {
            $quizinfo[get_string('duration', 'extendedquiz_statistics')] =
        ...
        foreach ($todisplay as $property => $format) {
            ...
            switch ($format) {
                case 'summarks_as_percentage':
                    $formattedvalue = extendedquiz_report_scale_summarks_as_percentage($value, $quiz);
                    break;
                case 'number_format_percent':
                    $formattedvalue = extendedquiz_format_grade($quiz, $value) . '%';
                    break;
                case 'number_format':
            ...
            $quizinfo[get_string($property, 'extendedquiz_statistics',
    ...
    protected function download_quiz_info_table($quizinfo) {
        ...
        // Do the output.
        $exportclass = $this->table->export_class_instance();
        $exportclass->start_table(get_string('quizinformation', 'extendedquiz_statistics'));
    ...
    protected function output_statistics_graph($quizstatsid, $s) {
        ...
        $imageurl = new moodle_url('/mod/extendedquiz/report/statistics/statistics_graph.php',
                array('id' => $quizstatsid));
        $graphname = get_string('statisticsreportgraph', 'extendedquiz_statistics');
    ...
    protected function compute_stats($quizid, $currentgroup, $nostudentsingroup,
        ...
        $qstats = new extendedquiz_statistics_question_stats($questions, $s, $summarksavg);
    ...
    protected function try_loading_cached_stats($quiz, $currentgroup,
        ...
        $quizstats = $DB->get_record_select('extendedquiz_statistics',
        ...
        $subquestions = array();
        $questionstats = $DB->get_records('extendedquiz_q_statistics',
    ...
    protected function cache_stats($quizid, $currentgroup,
        ...
        // Store the data.
        $quizstats->id = $DB->insert_record('extendedquiz_statistics', $toinsert);

        foreach ($questions as $question) {
            $question->_stats->quizstatisticsid = $quizstats->id;
            $DB->insert_record('extendedquiz_q_statistics', $question->_stats, false);
        }

        foreach ($subquestions as $subquestion) {
            $subquestion->_stats->quizstatisticsid = $quizstats->id;
            $DB->insert_record('extendedquiz_q_statistics', $subquestion->_stats, false);
        }
    ...
    protected function everything_download_options() {
        ...
        $output .= html_writer::tag('label', get_string('downloadeverything', 'extendedquiz_statistics', $downloadelements));
        $output .= '</div></form>';
    ...
    protected function output_caching_info($quizstats, $quizid, $currentgroup,
        ...
        $output .= get_string('lastcalculated', 'extendedquiz_statistics', $a);
        $output .= $OUTPUT->single_button($recalcualteurl,
                get_string('recalculatenow', 'extendedquiz_statistics'));
    ...
    protected function clear_cached_data($quizid, $currentgroup, $useallattempts) {
        ...
        $todelete = $DB->get_records_menu('extendedquiz_statistics', array('quizid' => $quizid,
        ...
        $DB->delete_records_select('extendedquiz_q_statistics',
                'quizstatisticsid ' . $todeletesql, $todeleteparams);
        $DB->delete_records_select('extendedquiz_q_resp_stats',
                'quizstatisticsid ' . $todeletesql, $todeleteparams);
        $DB->delete_records_select('extendedquiz_statistics',
    ...
    protected function using_attempts_string($useallattempts) {
        if ($useallattempts) {
            return get_string('allattempts', 'extendedquiz_statistics');
        } else {
            return get_string('firstattempts', 'extendedquiz_statistics');
        }
    }
}
...
function quiz_statistics_attempts_sql($quizid, $currentgroup, $groupstudents,
    ...
    $fromqa = '{extendedquiz_attempts} quiza ';

    $whereqa = 'quiza.quiz = :quizid AND quiza.preview = 0 AND quiza.state = :quizstatefinished';
    $qaparams = array('quizid' => $quizid, 'quizstatefinished' => extendedquiz_attempt::FINISHED);
