 ...
 *
 * It takes one parameter, the extendedquiz_statistics.id. This is enough to identify the
 * quiz etc.
 *
...
$quizstatistics = $DB->get_record('extendedquiz_statistics', array('id' => $quizstatisticsid));
$quiz = $DB->get_record('extendedquiz', array('id' => $quizstatistics->quizid), '*', MUST_EXIST);
$cm = get_coursemodule_from_instance('extendedquiz', $quiz->id);

// Check access.
require_login($quiz->course, false, $cm);
$modcontext = context_module::instance($cm->id);
require_capability('extendedquiz/statistics:view', $modcontext);
...
$questionstatistics = $DB->get_records_select('extendedquiz_q_statistics',
...
$graph->parameter['x_label'] = get_string('position', 'extendedquiz_statistics');
...
$fieldstoplot = array(
    ...
    'discriminativeefficiency' => get_string('discriminative_efficiency', 'extendedquiz_statistics')
);
