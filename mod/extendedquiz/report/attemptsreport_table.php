...
/**
 * Base class for the table used by a {@link extendedquiz_attempts_report}.
 *
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class extendedquiz_attempts_report_table extends table_sql {
     ...
     * @param mod_extendedquiz_attempts_report_options $options
    ...
    public function __construct($uniqueid, $quiz, $context, $qmsubselect,
            mod_extendedquiz_attempts_report_options $options, $groupstudents, $students,
            $questions, $reporturl) {
    ...
    public function col_fullname($attempt) {
        ...
        return $html . html_writer::empty_tag('br') . html_writer::link(
                new moodle_url('/mod/extendedquiz/review.php', array('attempt' => $attempt->attempt)),
    ...
    public function col_state($attempt) {
        if (!is_null($attempt->attempt)) {
            return extendedquiz_attempt::state_name($attempt->state);
        } else {
            return  '-';
        }
    }
    ...
    public function col_feedbacktext($attempt) {
        ...
        $feedback = extendedquiz_report_feedback_for_grade(
                extendedquiz_rescale_grade($attempt->sumgrades, $this->quiz, false),
    ...
    public function make_review_link($data, $attempt, $slot) {
        ...
        $url = new moodle_url('/mod/extendedquiz/reviewquestion.php',
    ...
    public function base_sql($reportstudents) {
        ...
        $from = "\n{user} u";
        $from .= "\nLEFT JOIN {extendedquiz_attempts} quiza ON
        ...
        switch ($this->options->attempts) {
            case extendedquiz_attempts_report::ALL_WITH:
                // Show all attempts, including students who are no longer in the course.
                $where = 'quiza.id IS NOT NULL AND quiza.preview = 0';
                break;
            case extendedquiz_attempts_report::ENROLLED_WITH:
                ...
                break;
            case extendedquiz_attempts_report::ENROLLED_WITHOUT:
                ...
                break;
            case extendedquiz_attempts_report::ENROLLED_ALL:
    ...
    protected function add_latest_state_join($slot) {
        ...
        $qubaids = new qubaid_join("{extendedquiz_attempts} {$alias}quiza", "{$alias}quiza.uniqueid",
    ...
    protected function submit_buttons() {
        global $PAGE;
        if (has_capability('mod/extendedquiz:deleteattempts', $this->context)) {
            echo '<input type="submit" id="deleteattemptsbutton" name="delete" value="' .
                    get_string('deleteselected', 'extendedquiz_overview') . '"/>';
