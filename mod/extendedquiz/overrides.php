...
$url = new moodle_url('/mod/extendedquiz/overrides.php', array('cmid'=>$cm->id, 'mode'=>$mode));
...
$sql = 'SELECT o.id
            FROM {extendedquiz_overrides} o LEFT JOIN {groups} g
            ON o.groupid = g.id
...
if (!empty($orphaned)) {
    $DB->delete_records_list('extendedquiz_overrides', 'id', array_keys($orphaned));
}
...
$overridedeleteurl = new moodle_url('/mod/extendedquiz/overridedelete.php');
$overrideediturl = new moodle_url('/mod/extendedquiz/overrideedit.php');
...
foreach ($overrides as $override) {
    ...
    if (!$groupmode) {
        if (!has_capability('mod/extendedquiz:attempt', $context, $override->userid)) {
...
if ($groupmode) {
    ...
    if (!empty($CFG->enablegroupmembersonly) && $cm->groupmembersonly) {
        ...
        if (!empty($limitgroups)) {
            $users = get_users_by_capability($context, 'mod/extendedquiz:attempt', 'u.id',
