...
require_once($CFG->dirroot . '/mod/extendedquiz/report/default.php');
...
$url = new moodle_url('/mod/extendedquiz/report.php', array('id' => $cm->id));
if ($mode !== '') {
    $url->param('mode', $mode);
}
$PAGE->set_url($url);
...
if (!is_readable("report/$mode/report.php")) {
    print_error('reportnotfound', 'extendedquiz', '', $mode);
}

add_to_log($course->id, 'extendedquiz', 'report', 'report.php?id=' . $cm->id . '&mode=' . $mode,
        $quiz->id, $cm->id);

// Open the selected quiz report and display it.
$file = $CFG->dirroot . '/mod/extendedquiz/report/' . $mode . '/report.php';
if (is_readable($file)) {
    include_once($file);
}
$reportclassname = 'extendedquiz_' . $mode . '_report';                 
if (!class_exists($reportclassname)) {
    print_error('preprocesserror', 'extendedquiz');
}
