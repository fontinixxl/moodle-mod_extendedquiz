...
$url = new moodle_url('/mod/extendedquiz/review.php', array('attempt'=>$attemptid));
if ($page !== 0) {
    $url->param('page', $page);
}
if ($showall !== 0) {
...
if ($attemptobj->is_own_attempt()) {
    if (!$attemptobj->is_finished()) {
        ...
        $accessmanager->back_to_view_page($PAGE->get_renderer('mod_extendedquiz'),
                $attemptobj->cannot_review_message());
    }

} else if (!$attemptobj->is_review_allowed()) {
    throw new moodle_extendedquiz_exception($attemptobj->get_quizobj(), 'noreviewattempt');
}
...
// Log this review.
add_to_log($attemptobj->get_courseid(), 'extendedquiz', 'review', 'review.php?attempt=' .
...
$summarydata['state'] = array(
    ...
    'content' => extendedquiz_attempt::state_name($attempt->state),
);
...
$grade = extendedquiz_rescale_grade($attempt->sumgrades, $quiz, false);
if ($options->marks >= question_display_options::MARK_AND_MAX && extendedquiz_has_grades($quiz)) {
    ...
    if ($attempt->state != extendedquiz_attempt::FINISHED) {
        ...
        $summarydata['grade'] = array(
            'title'   => get_string('grade', 'quiz'),
            'content' => extendedquiz_format_grade($quiz, $grade),
        );
        ...
        if ($quiz->grade != $quiz->sumgrades) {
            $a = new stdClass();
            $a->grade = extendedquiz_format_grade($quiz, $attempt->sumgrades);
            $a->maxgrade = extendedquiz_format_grade($quiz, $quiz->sumgrades);
        ...
        $a = new stdClass();
        $a->grade = html_writer::tag('b', extendedquiz_format_grade($quiz, $grade));
...
$navbc = $attemptobj->get_navigation_panel($output, 'extendedquiz_review_nav_panel', $page, $showall); //?¿?¿?¿?
