...
if (!$quizobj->has_questions()) {
    if ($quizobj->has_capability('mod/extendedquiz:manage')) {
        redirect($quizobj->edit_url());
    } else {
        print_error('cannotstartnoquestions', 'extendedquiz', $quizobj->view_url());
    }
}
...
if (!$quizobj->is_preview_user()) {
    $quizobj->require_capability('mod/extendedquiz:attempt');
}
...
if ($quizobj->is_preview_user() && $forcenew) {
    ...
    $DB->set_field('extendedquiz_attempts', 'state', extendedquiz_attempt::FINISHED,
            array('quiz' => $quizobj->get_quizid(), 'userid' => $USER->id));
}

// Look for an existing attempt.
$attempts = extendedquiz_get_user_attempts($quizobj->get_quizid(), $USER->id, 'all', true);
$lastattempt = end($attempts);

// If an in-progress attempt exists, check password then redirect to it.
if ($lastattempt && ($lastattempt->state == extendedquiz_attempt::IN_PROGRESS ||
        $lastattempt->state == extendedquiz_attempt::OVERDUE)) {
    ...
    if ($lastattempt->state == extendedquiz_attempt::OVERDUE) {
        redirect($quizobj->summary_url($lastattempt->id));
    } else if ($lastattempt->state != extendedquiz_attempt::IN_PROGRESS) {
...
if (!$quizobj->is_preview_user() && $messages) {
    print_error('attempterror', 'extendedquiz', $quizobj->view_url(),
...
extendedquiz_delete_previews($quizobj->get_quiz(), $USER->id);

$quba = question_engine::make_questions_usage_by_activity('mod_extendedquiz', $quizobj->get_context());
$quba->set_preferred_behaviour($quizobj->get_quiz()->preferredbehaviour);

// Create the new attempt and initialize the question sessions
$timenow = time(); // Update time now, in case the server is running really slowly.
$attempt = extendedquiz_create_attempt($quizobj, $attemptnumber, $lastattempt, $timenow,
...
if (!($quizobj->get_quiz()->attemptonlast && $lastattempt)) {
    ...
    foreach ($quizobj->get_questions() as $i => $questiondata) {
        if ($questiondata->qtype != 'random') {
            ...
            if (is_null($question)) {
                throw new moodle_exception('notenoughrandomquestions', 'extendedquiz',
...
$attempt->id = $DB->insert_record('extendedquiz_attempts', $attempt);

// Log the new attempt.
if ($attempt->preview) {
    add_to_log($course->id, 'extendedquiz', 'preview', 'view.php?id=' . $quizobj->get_cmid(),
            $quizobj->get_quizid(), $quizobj->get_cmid());
} else {
    add_to_log($course->id, 'extendedquiz', 'attempt', 'review.php?attempt=' . $attempt->id,
            $quizobj->get_quizid(), $quizobj->get_cmid());
}
